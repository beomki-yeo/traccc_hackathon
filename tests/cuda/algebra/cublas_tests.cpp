...
TEST(algebra, cublas_tests) {
    ...
    m_status = cublasDgemm(m_handle, CUBLAS_OP_N, CUBLAS_OP_N, A.rows(),
                           B.cols(), B.rows(), &alpha, &A_dev.ptr()[0](0),
                           A.rows(), &B_dev.ptr()[0](0), B.rows(), &beta,
    ...
    m_status =
        cublasDgemm(m_handle, CUBLAS_OP_N, CUBLAS_OP_N, A.rows(), B.cols(),
                    B.rows(), &alpha, &A_mng[0](0), A.rows(), &B_mng[0](0),
                    B.rows(), &beta, &C_mng[0](0), C.rows());

    std::cout << std::endl;
    std::cout << "Test case 2) A:unified B:unified C:unified (Does NOT work "
                 "correctly)"
              << std::endl;
    ...
    C_dev = m_copy.to(vecmem::get_data(C_host), dev_mr,
                      vecmem::copy::type::host_to_device);

    m_status =
        cublasDgemm(m_handle, CUBLAS_OP_N, CUBLAS_OP_N, A.rows(), B.cols(),
                    B.rows(), &alpha, &A_mng[0](0), A.rows(), &B_mng[0](0),
                    B.rows(), &beta, &C_dev.ptr()[0](0), C.rows());
    ...
    std::cout << std::endl;
    std::cout
        << "Test case 4) A:device B:device C:unified (Does NOT work correctly)"
        << std::endl;
    std::cout << C_mng[0] << std::endl;
