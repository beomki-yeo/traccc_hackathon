...
TEST(algebra, matmult_tests) {
    ...
    /*time*/ std::chrono::duration<double> time_cpu_managed =
        end_cpu_managed - start_cpu_managed;
    ...
    for (size_t i_n = 0; i_n < n_event; i_n++) {
        matmul(batch_size, vecmem::get_data(A), vecmem::get_data(B),
               vecmem::get_data(C_gpu), gpu_managed);
    }
    ...
    /*time*/ std::chrono::duration<double> time_cpu_host =
        end_cpu_host - start_cpu_host;
    ...
    for (size_t i_n = 0; i_n < n_event; i_n++) {
        ...
        matmul(batch_size, A_dev, B_dev, C_dev, gpu_device);
    }
    // take average across the events
