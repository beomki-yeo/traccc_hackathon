...
TEST(algebra, gain_matrix) {
    ...
    using track_state_t =
        traccc::track_state<measurement_t, bound_track_parameters_t>;
    using host_track_state_collection =
        traccc::host_track_state_collection<track_state_t>;

    // declare a test track states object
    host_track_state_collection track_states_cpu(
        {host_track_state_collection::item_vector(&mng_mr)});
    host_track_state_collection track_states_gpu(
        {host_track_state_collection::item_vector(&mng_mr)});
    ...
    for (int i_b = 0; i_b < batch_size; i_b++) {
        ...
        tr_state.predicted().vector() =
            bound_track_parameters_t::vector_t::Random();
        tr_state.predicted().covariance() =
            bound_track_parameters_t::covariance_t::Random();

        tr_state.filtered().vector() =
            bound_track_parameters_t::vector_t::Random();
        tr_state.filtered().covariance() =
            bound_track_parameters_t::covariance_t::Random();

        tr_state.smoothed().vector() =
            bound_track_parameters_t::vector_t::Random();
        tr_state.smoothed().covariance() =
            bound_track_parameters_t::covariance_t::Random();
