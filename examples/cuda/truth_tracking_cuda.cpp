...
int seq_run(const std::string& detector_file, const std::string& hits_dir,
    ...
    for (unsigned int event = skip_events; event < skip_events + events;
        ...
        std::string io_particle_file = hits_dir + std::string("/event") +
                                       event_string +
                                       std::string("-particles_initial.csv");
        ...
        traccc::host_truth_measurement_container measurements_per_event(
            {traccc::host_truth_measurement_container::header_vector(
                 n_particles, &mng_mr),
        ...
        traccc::host_truth_bound_track_parameters_container
            bound_track_parameters_per_event(
                {traccc::host_truth_bound_track_parameters_container::
                     header_vector(n_particles, &mng_mr),
                 traccc::host_truth_bound_track_parameters_container::
                     item_vector(n_particles, &mng_mr)});
        ...
        for (unsigned int i_h = 0; i_h < spacepoints_per_event.headers.size();
            ...
            for (auto sp : spacepoints_per_particle) {
                ...
                auto params =
                    sp.make_bound_track_parameters(surfaces, t_particle);
        ...
        /*----------------------------------------------------
          ...
          (1.A) header is the vector of truth particle which contains particle
          id, particle type, mass and vertex information (1.B) item is the
          vector of vector of measurement where each subvector is associated
          with each element of header. you can access surface by using
          surface_id member varaible ex) measurement ms; surface reference_surf
          = surfaces.items[ms.surface_id];

          (2) bound_track_parameter container
          (2.A) header is the vector of truth particle which contains particle
          id, particle type, mass and vertex information -> same with
          measurement container (2.B) item is the vector of vector of
          bound_track_parameter consisting of vector (2-dim position, mom, time)
          and its covariance (+ surface_id)
