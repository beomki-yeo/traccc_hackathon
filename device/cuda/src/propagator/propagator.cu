...
__global__ void propagate_kernel(collection_view<state_t> states_view,
...
template void truth_propagator ::propagate<void_multi_state, surface>(
...
void traccc::cuda::propagator<stepper_t, navigator_t>::propagate(
    state_t& state, host_collection<surface_t>& surfaces,
    vecmem::memory_resource* resource) {
    ...
    // run the kernel
    propagate_kernel<typename state_t::state_t, surface_t>
