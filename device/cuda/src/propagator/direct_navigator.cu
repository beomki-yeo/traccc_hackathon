...
template < typename navigator_state_t,
	   typename stepper_state_t,
	   typename surface_t >
__global__ void status_kernel(collection_view<navigator_state_t> navigator_state_view,
			      collection_view<stepper_state_t> stepper_state_view,
			      collection_view<surface_t> surface_view);
        
// explicit instantiation
template
bool direct_navigator::status<typename eigen_stepper::state, surface>
...
bool direct_navigator::status(host_collection<state>& navigator_state,
    ...
    unsigned int num_blocks = navigator_state_view.items.size()/num_threads+1;

    // run the kernel
    status_kernel< state, stepper_state_t, surface_t ><<<num_blocks, num_threads>>>(navigator_state_view, stepper_state_view, surfaces_view);
...
__global__ void status_kernel(collection_view<navigator_state_t> navigator_state_view,
    ...
    device_collection<navigator_state_t> navigator_state_device({navigator_state_view.items});
    device_collection<stepper_state_t> stepper_state_device({stepper_state_view.items});
