...
class propagator final {
    ...
    struct state {
        ...
        state(const int n_tracks, vecmem::memory_resource* mr)
            :
	    m_mr(mr),
	    options({typename host_collection<propagator_options_t>::item_vector(n_tracks, mr)}),
	    stepping({typename host_collection<typename stepper_t::state>::item_vector(n_tracks, mr)}),
	    navigation({typename host_collection<typename navigator_t::state>::item_vector(n_tracks, mr)})

	{}
	
	auto& get_options() { return options;}
	auto& get_stepping() { return stepping;}
	auto& get_navigation() { return navigation;}
	...
	return propagate(state.options,
			 state.stepping,
			 state.navigation,
			 surfaces, resource);	
    }
