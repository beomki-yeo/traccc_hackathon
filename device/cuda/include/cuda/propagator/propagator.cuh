...
class propagator final {
    ...
    struct __CUDA_ALIGN__(16) multi_state {
        ...
        using propagator_t =
            typename traccc::propagator<stepper_t, navigator_t>;
        using state_t =
            typename propagator_t::template state<propagator_options_t>;

        multi_state(const int n_tracks, vecmem::memory_resource* mr)
            : states({typename host_collection<state_t>::item_vector(n_tracks,
                                                                     mr)})
