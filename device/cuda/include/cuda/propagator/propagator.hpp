...
class propagator final {
    ...
    struct state {
        ...
        state(const params_container_t& start, const propagator_options_t& tops,
              vecmem::memory_resource* mr)
            : options(tops),
              stepping({typename host_collection<stepper_state_t>::item_vector(
                  start.size(), mr)}),
              navigation(
                  {typename host_collection<navigator_state_t>::item_vector(
                      start.size(), mr)}) {}
