...
static __CUDA_HOST_DEVICE__ void reinitialize_jacobians(
    Acts::FreeMatrix& free_transport_jacobian,
    Acts::FreeVector& free_to_path_derivatives,
    Acts::BoundToFreeMatrix& bound_to_free_jacobian,
    const Acts::FreeVector& free_vector, const surface& surface) {
    // Reset the jacobians
    ...
    Acts::BoundVector bound_vector =
        detail::transform_free_to_bound_parameters(free_vector, surface);

    // Reset the jacobian from local to global
    bound_to_free_jacobian = surface.bound_to_free_jacobian(bound_vector);
}

static __CUDA_HOST_DEVICE__ void bound_to_bound_jacobian(
    ...
    Acts::BoundMatrix& full_transport_jacobian, const surface& surface) {
    ...
    full_transport_jacobian = free_to_bound_jacobian *
                              (Acts::FreeMatrix::Identity() +
                               free_to_path_derivatives * free_to_path) *
                              free_transport_jacobian * bound_to_free_jacobian;
}

static __CUDA_HOST_DEVICE__ void transport_covariance_to_bound(
    Acts::BoundSymMatrix& boundCovariance,
    Acts::BoundMatrix& fullTransportJacobian,
    Acts::FreeMatrix& freeTransportJacobian,
    ...
    reinitialize_jacobians(freeTransportJacobian, freeToPathDerivatives,
                           boundToFreeJacobian, freeParameters, surface);
}

static __CUDA_HOST_DEVICE__ std::tuple<
    bound_track_parameters, typename bound_track_parameters::jacobian_t, double>
bound_state(Acts::BoundSymMatrix& covarianceMatrix, Acts::BoundMatrix& jacobian,
            Acts::FreeMatrix& transportJacobian, Acts::FreeVector& derivatives,
            Acts::BoundToFreeMatrix& boundToFreeJacobian,
            const Acts::FreeVector& parameters, bool covTransport,
            double accumulatedPath, const int& surface_id,
    ...
    if (covTransport) {
        ...
        // Calculate the jacobian and transport the covarianceMatrix to final
        // local. Then reinitialize the transportJacobian, derivatives and the
        // boundToFreeJacobian
    ...
    return std::make_tuple(bound_track_parameters({bv, cov, surface_id}),
