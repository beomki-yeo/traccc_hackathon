...
public:
    ...
    static __CUDA_HOST_DEVICE__ void status(state& state,
					    stepper_t& stepper_state,
					    host_collection<surface_t>& surfaces){  
	...
	if (state.surface_iterator_id < state.surface_sequence_size){
	    ...
	    auto surface_status = stepping_helper::update_surface_status(stepper_state, target_surface);
		...
		state.target_surface_id = state.surface_sequence[state.surface_iterator_id];
	    }	   
	}
    }

    template <typename propagator_state_t, typename stepper_t>
    static void target(propagator_state_t& state,
